...
The SaaS Platform UI is a dedicated administrative interface for platform admins to manage the entire multi-tenant SaaS platform. It provides comprehensive oversight and control over users, organizations, subscriptions, and system health across all tenants.
...
- `PLATFORM_ADMIN` authority bypasses all tenant restrictions for cross-tenant administrative operations
...
- `POST /api/v1/billing/plans` - Create plan (PLATFORM_ADMIN)
- `PUT /api/v1/billing/plans/{planCode}` - Update plan (PLATFORM_ADMIN)
- `DELETE /api/v1/billing/plans/{planCode}` - Deactivate plan (PLATFORM_ADMIN)

### 📋 Planned APIs

**User Admin Actions:**

- `POST /api/v1/iam/admin/users/{id}/ban` - Ban user (PLATFORM_ADMIN)
- `POST /api/v1/iam/admin/users/{id}/unban` - Unban user (PLATFORM_ADMIN)
- `POST /api/v1/iam/admin/users/{id}/impersonate` - Impersonate user (PLATFORM_ADMIN)
- `POST /api/v1/iam/admin/users/{id}/unlock` - Unlock account (PLATFORM_ADMIN)
- `POST /api/v1/iam/admin/users/{id}/verify-email` - Verify email (PLATFORM_ADMIN)
- `GET /api/v1/iam/admin/users/{id}/memberships` - Get memberships (PLATFORM_ADMIN)
- `GET /api/v1/iam/admin/users/{id}/activity` - Get activity log (PLATFORM_ADMIN)

**Tenant Admin Actions:**

- `GET /api/v1/iam/admin/tenants` - List all tenants (paginated, PLATFORM_ADMIN)
- `GET /api/v1/iam/admin/tenants/{tenantKey}` - Get tenant (platform admin view, PLATFORM_ADMIN)
- `POST /api/v1/iam/admin/tenants/{tenantKey}/suspend` - Suspend tenant (PLATFORM_ADMIN)
- `POST /api/v1/iam/admin/tenants/{tenantKey}/unsuspend` - Unsuspend tenant (PLATFORM_ADMIN)
- `DELETE /api/v1/iam/admin/tenants/{tenantKey}` - Delete tenant (PLATFORM_ADMIN)
- `POST /api/v1/iam/admin/tenants/{tenantKey}/transfer-ownership` - Transfer ownership (PLATFORM_ADMIN)
- `GET /api/v1/iam/admin/tenants/{tenantKey}/export` - Export data (GDPR, PLATFORM_ADMIN)

**Subscription Admin Actions:**

- `GET /api/v1/billing/admin/subscriptions` - List all subscriptions (PLATFORM_ADMIN)
- `POST /api/v1/billing/admin/subscriptions/{id}/change-plan` - Change plan (PLATFORM_ADMIN)
- `POST /api/v1/billing/admin/subscriptions/{id}/cancel` - Cancel subscription (PLATFORM_ADMIN)
- `POST /api/v1/billing/admin/subscriptions/{id}/reactivate` - Reactivate (PLATFORM_ADMIN)
- `POST /api/v1/billing/admin/subscriptions/{id}/apply-discount` - Apply discount (PLATFORM_ADMIN)
- `POST /api/v1/billing/admin/subscriptions/{id}/extend-trial` - Extend trial (PLATFORM_ADMIN)

**System Administration:**

- `GET /api/v1/iam/admin/dashboard/metrics` - Platform metrics (PLATFORM_ADMIN)
- `GET /api/v1/iam/admin/audit-log` - Audit trail (PLATFORM_ADMIN)
- `GET /api/v1/iam/admin/system/health` - Service health (PLATFORM_ADMIN)
- `GET /api/v1/iam/admin/system/jobs` - Background jobs (PLATFORM_ADMIN)
- `POST /api/v1/iam/admin/system/jobs/{jobName}/trigger` - Trigger job (PLATFORM_ADMIN)
...
- Authority in each organization (TENANT_OWNER, ADMIN, MEMBER)
...
**Platform Admin Actions (Current Implementation):**
...
**Platform Admin Actions (To Be Implemented):**
...
- Inline actions: Change authority, Remove member
- Bulk actions: Change authortities, Send notifications
...
- **Create Plan:** `POST /api/v1/billing/plans` - Create new plan (PLATFORM_ADMIN)
- **Update Plan:** `PUT /api/v1/billing/plans/{planCode}` - Update plan details (PLATFORM_ADMIN)
- **Deactivate Plan:** `DELETE /api/v1/billing/plans/{planCode}` - Soft delete by setting active=false (PLATFORM_ADMIN)
...
- Global audit trail of all platform admin actions
...
- Platform operators authenticate through the same login flow as regular users — there is no separate login page
- After login, the JWT contains `PLATFORM_ADMIN` authority; the `_operator` route group guard detects this and grants access to `/admin/*` routes
- MFA required for all platform admin accounts
- Session timeout: 30 minutes of inactivity (same session infrastructure as tenant users)
- IP whitelist can be enforced at the API Gateway level for `/api/v1/iam/admin/*` and `/api/v1/billing/admin/*` endpoints

**Token Storage Strategy:**

- Access token stored in **memory only** (JavaScript variable in the `session` Zustand store) — never in `localStorage` or `sessionStorage`, never in a non-httpOnly cookie. This applies equally to tenant users and platform admins; there is no separate token store for operators.
...
- This strategy means the app has no persistent token in JavaScript-accessible storage — XSS cannot steal a long-lived credential regardless of which surface the user is on.
...
1. Check if user has PLATFORM_ADMIN authority
...
- `PLATFORM_ADMIN` authority required for plan catalog mutations (POST/PUT/DELETE)
- `TENANT_OWNER` authority required for tenant management endpoints (when not platform admin)
- `TENANT_OWNER` or `ADMIN` authority required for invitation management (when not platform admin)
- User admin endpoints (`/api/v1/iam/admin/users`) should be secured with `PLATFORM_ADMIN` authority
...
@PreAuthorize("hasAuthority('PLATFORM_ADMIN') or " +
...
- **Platform Authority:** `PLATFORM_ADMIN` bypasses tenant isolation for all administrative operations
- **Tenant-Scoped Authorities:** `TENANT_OWNER`, `PLATFORM_ADMIN`, `MEMBER` are restricted to their tenant context
...
**Platform Admin Authorities:**

```
PLATFORM_ADMIN (bypasses all tenant restrictions)
...
```java
// Check for platform admin authority first, then fall back to tenant-scoped
if (hasPlatformAdminAuthority(jwt)) {
    // Bypass tenant validation - platform admin can access any tenant
...
- Gateway validates `PLATFORM_ADMIN` authority before adding header

3. **Audit Requirements:**

- All platform admin actions logged with:
...
**Platform Admin API Endpoints (To Be Implemented):**
...
GET    /api/v1/iam/admin/tenants/{tenantKey}      # Get tenant details (platform admin view)
...
# Admin Preferences
GET    /api/v1/iam/admin/admins/me/preferences # Get operator preferences (locale, UI settings)
PATCH  /api/v1/iam/admin/admins/me/preferences # Save operator preferences
...
Every route sets a descriptive `<title>` via `@dr.pogodin/react-helmet` using the `<PageTitle>` component from `shared/lib/page-title`. Titles follow a **breadcrumb pattern** — segments run from the most-specific (leaf) page on the left to the app name on the right, separated by `|`.

**App name constant for this surface:** `ADMIN_TITLE = "Key Value Admin"`

**Pattern:** `<Leaf> | <Parent Section> | Key Value Admin`

| Route                                      | Rendered title                                              |
| ------------------------------------------ | ----------------------------------------------------------- |
| `/admin`                                   | `Dashboard \| Key Value Admin`                              |
| `/admin/users`                             | `Users \| Key Value Admin`                                  |
| `/admin/users/$id` (Profile tab)           | `Alice Smith \| Users \| Key Value Admin`                   |
| `/admin/users/$id/memberships`             | `Memberships \| Alice Smith \| Users \| Key Value Admin`    |
| `/admin/users/$id/activity`                | `Activity \| Alice Smith \| Users \| Key Value Admin`       |
| `/admin/organizations`                     | `Organizations \| Key Value Admin`                          |
| `/admin/organizations/$tenantKey`          | `Acme Corp \| Organizations \| Key Value Admin`             |
| `/admin/organizations/$tenantKey/members`  | `Members \| Acme Corp \| Organizations \| Key Value Admin`  |
| `/admin/organizations/$tenantKey/billing`  | `Billing \| Acme Corp \| Organizations \| Key Value Admin`  |
| `/admin/organizations/$tenantKey/activity` | `Activity \| Acme Corp \| Organizations \| Key Value Admin` |
| `/admin/subscriptions`                     | `Subscriptions \| Key Value Admin`                          |
| `/admin/subscriptions/$id`                 | `sub_xxx \| Subscriptions \| Key Value Admin`               |
| `/admin/plans`                             | `Plans \| Billing \| Key Value Admin`                       |
| `/admin/plans/$planCode`                   | `Pro Monthly \| Plans \| Billing \| Key Value Admin`        |
| `/admin/system/health`                     | `Health \| System \| Key Value Admin`                       |
| `/admin/system/jobs`                       | `Jobs \| System \| Key Value Admin`                         |
| `/admin/audit-log`                         | `Audit Log \| Key Value Admin`                              |

**Rules (enforced, see UI Coding Guidelines for full spec):**

- Every page component renders `<PageTitle segments={[...]} appTitle={ADMIN_TITLE} />`.
- While entity data is loading, use a generic fallback (`"User Details"`, `"Organization Details"`).
- Active tab name is the leftmost segment on tabbed detail pages.
- All static segments are wrapped in Lingui `_(msg\`...\`)` for i18n extraction.
...
The SaaS Platform UI provides full internationalization support using **Lingui** for multi-language admin interfaces, enabling platform admins worldwide to use the admin panel in their preferred language.
...
// PATCH /api/v1/iam/admin/admins/me/preferences
// { "locale": "ru" }
...
- Admin routes (`/admin/*`) are protected by a single `beforeLoad` guard on the `_operator.tsx` layout route. The guard checks `PLATFORM_ADMIN` authority from the decoded JWT in the `operator-session` Zustand store.
- Tenant routes are protected by a separate `beforeLoad` guard on the `_tenant.tsx` layout route that checks authentication only.
- MFA enforcement for all platform admin accounts.
- IP whitelist can be applied at the API Gateway level for `/api/v1/iam/admin/*` and `/api/v1/billing/admin/*` endpoints — this provides network-level isolation without requiring a separate deployment.

**Platform Authority Bypass:**

- `PLATFORM_ADMIN` bypasses all tenant isolation checks
- Gateway propagates `X-Platform-Operator: true` header for platform admins
- Downstream services check platform admin authority before enforcing tenant context validation
...
- Complete audit trail of all platform admin actions
...
- Platform operator API endpoints rate-limited per platform admin
...
- ✅ Plan catalog API with PLATFORM_ADMIN authority
...
- Basic platform admin actions (ban, suspend, delete) with confirmation dialogs
- Audit log viewer
...
> Strict design and architecture rules for the UI codebase.
...
The operator surface is **not a separate application**. It is a protected route group (`/admin/*`) embedded in the same SPA as the tenant-facing UI. Both surfaces share one build, one deployment, and one API Gateway connection.

The two surfaces are isolated by:

- **Route groups** — TanStack Router pathless layout routes (`_tenant` and `_operator`) each own their `AppShell`, their `beforeLoad` guard, and their navigation structure.
- **Session stores** — two separate Zustand stores: `session` (tenant user) and `operator-session` (platform admin + impersonation state). Neither store is accessible to the other surface's components.
- **API Gateway** — `/api/v1/iam/admin/*` and `/api/v1/billing/admin/*` endpoints require `PLATFORM_ADMIN` authority server-side regardless of what the UI sends.

This approach trades network-level isolation (separate origin, VPN-only access) for significantly lower operational overhead. The tradeoff is acceptable for early-stage platforms. Extraction into a separate SPA is straightforward if network isolation becomes a compliance requirement later.

---

### Layer Structure — Feature-Sliced Design (FSD)

The codebase strictly follows [Feature-Sliced Design](https://feature-sliced.design/). Layers are ordered top → bottom. A layer may only import from layers **below** it. Cross-layer imports must go through the public API (`index.ts`) — never import internal files directly.
...
- Passes `queryClient` into the router context so `beforeLoad` guards can call `ensureQueryData`.
...
The route tree is split into two pathless layout groups. A pathless route file (prefixed with `_`) adds a layout and a `beforeLoad` guard without contributing a URL segment.

```
pages/
├── __root.tsx                          # Pure provider shell — no layout, just <Outlet />
├── login.tsx                           # /login — outside both groups, no AppShell
│
├── _tenant.tsx                         # Pathless layout: tenant AppShell + auth guard
├── _tenant.index.tsx                   # / → tenant dashboard
├── _tenant.settings.tsx                # /settings
│
├── _operator.tsx                       # Pathless layout: operator AppShell + PLATFORM_ADMIN guard
├── _operator.index.tsx                 # /admin → metrics dashboard
├── _operator.users.index.tsx           # /admin/users
├── _operator.users.$userId.tsx         # /admin/users/:userId
├── _operator.organizations.index.tsx   # /admin/organizations
├── _operator.organizations.$tenantKey.tsx
├── _operator.subscriptions.index.tsx   # /admin/subscriptions
├── _operator.subscriptions.$id.tsx
├── _operator.plans.index.tsx           # /admin/plans
├── _operator.plans.$planCode.tsx
├── _operator.audit-log.tsx             # /admin/audit-log
└── _operator.system.tsx                # /admin/system
```

**`_tenant.tsx` — tenant layout route:**
...
**`_operator.tsx` — operator layout route:**
...
export const Route = createFileRoute("/_operator")({
  beforeLoad: ({ context }) => {
    ...
    if (!context.session.hasPlatformAdminAuthority) {
      throw redirect({ to: "/" });
    }
  },
  component: () => (
...
The `beforeLoad` guard on `_operator.tsx` covers every route under `/admin/*` — individual operator pages do not repeat the authority check.

**`__root.tsx` — minimal provider shell:**
...
No layout, no guard. Layout and guards live in the group files, not the root.
...
```
widgets/
├── tenant-shell/        # Tenant AppShell: sidebar nav, user menu, breadcrumbs
├── admin-shell/         # Admin AppShell: admin nav, impersonation banner, Cmd+K palette
├── tenant-dashboard/    # Tenant-facing dashboard (if applicable)
├── admin-dashboard/     # Admin metrics cards, charts, date range selector
...
```
features/
├── auth/                # Login flow, MFA, session bootstrap (shared entry point for both surfaces)
...
- One slice per backend domain: `user`, `organization`, `subscription`, `plan`, `audit-log`, `session`, `operator-session`.
...
- The `session` entity holds the current tenant user's JWT state. The `operator-session` entity holds the platform admin's JWT state and impersonation state. These are two separate Zustand stores — neither is accessible from the other surface's components.

```
entities/
├── user/
│   ├── api.ts           # useUsers (operator, cross-tenant), useUser, useCreateUser, useUpdateUser, useDeleteUser
│   ├── model.ts         # User type, UserStatus enum, status → color map
│   ├── ui.tsx           # UserStatusBadge
│   └── index.ts
├── organization/
│   ├── api.ts           # useAllOrganizations (operator), useOrganization, useTenantOrganization (tenant-scoped)
...
│   └── index.ts
├── session/
│   ├── model.ts         # TenantSession type, decoded JWT claims
│   ├── store.ts         # Zustand store — current tenant user session
│   └── index.ts
└── platform-admin-session/
    ├── model.ts         # PlatformAdminSession type, ImpersonationSession type
...
**The hard rule for `shared/`:** if a component or utility knows what a `TenantStatus`, `UserStatus`, or `PLATFORM_ADMIN` is — it does not belong in `shared/`. Move it to the appropriate entity.
...
| Zustand stores                    | `use` prefix + `Store` suffix | `useSessionStore`, `useOperatorSessionStore` |
...
- Current tenant user session and decoded JWT claims (`session` entity store).
- Current operator session and decoded JWT claims (`operator-session` entity store).
- Impersonation session state (active, target user, reason, expiry) — in `operator-session` store only.
...
- The `PLATFORM_ADMIN` authority check is performed once in `_operator.tsx`'s `beforeLoad`. This single guard covers every route under `/admin/*`. Individual operator pages do not repeat the check.
- Non-authenticated users hitting any protected route are redirected to `/login`. Authenticated users without `PLATFORM_ADMIN` authority hitting `/admin/*` are redirected to `/` (tenant dashboard).
- JWT claims are read from the decoded token in the `session` or `operator-session` Zustand store — never from `localStorage` directly.
- Impersonation state is visually indicated in the `operator-shell` widget at all times when active. No action that modifies the impersonated user's password, billing, or account deletion is permitted during impersonation — these actions are disabled in the UI and rejected by the API.
...
| `_operator.tsx` layout contains `beforeLoad` guard | File content check for `beforeLoad` and `PLATFORM_ADMIN` |
| Admin pages live under `_operator` prefix          | Filename prefix check for all `/admin/*` routes          |
...
**Level 1 — Root route (`__root.tsx`): authentication + platform admin authority**
...
This is the only place that checks `isAuthenticated` and `hasPlatformAdminAuthority`. Do not repeat these checks in child routes.

**Level 2 — Route group layouts: sub-authority checks**

Route groups (TanStack Router layout routes) protect logical sections that require a specific operator authority beyond the base `PLATFORM_ADMIN`. Only add a level-2 guard when a section genuinely has a narrower permission requirement.
...
Read-only sections (`VIEW_*` authorities) that all platform admins have do not need a level-2 guard — the root guard is sufficient.
...
```ts
// entities/admin-session/store.ts
...
| Not `PLATFORM_ADMIN`                    | `beforeLoad` + `throw redirect`                            | `pages/__root.tsx`                                   |
