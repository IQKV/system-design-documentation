...
The SaaS Platform UI is a dedicated administrative interface for platform operators to manage the entire multi-tenant SaaS platform. It provides comprehensive oversight and control over users, organizations, subscriptions, and system health across all tenants.
...
**Architecture:** Operator pages embedded in the main user-facing SPA as a protected route group (`/operator/*`). No separate deployment — operator and tenant surfaces share the same app, the same build, and the same API Gateway connection. Route-level guards enforce `PLATFORM_OPERATOR` authority; the two surfaces have completely separate layouts and session stores.
...
- After login, the JWT contains `PLATFORM_OPERATOR` authority; the `_operator` route group guard detects this and grants access to `/operator/*` routes
...
- IP whitelist can be enforced at the API Gateway level for `/api/v1/iam/operator/*` and `/api/v1/billing/operator/*` endpoints

**Token Storage Strategy:**

- Access token stored in **memory only** (JavaScript variable in the `session` Zustand store) — never in `localStorage` or `sessionStorage`, never in a non-httpOnly cookie. This applies equally to tenant users and platform operators; there is no separate token store for operators.
...
| --------------------------------------------- | ----------------------------------------------------------- |
| `/operator`                                   | `Dashboard \| Key Value Admin`                              |
| `/operator/users`                             | `Users \| Key Value Admin`                                  |
| `/operator/users/$id` (Profile tab)           | `Alice Smith \| Users \| Key Value Admin`                   |
| `/operator/users/$id/memberships`             | `Memberships \| Alice Smith \| Users \| Key Value Admin`    |
| `/operator/users/$id/activity`                | `Activity \| Alice Smith \| Users \| Key Value Admin`       |
| `/operator/organizations`                     | `Organizations \| Key Value Admin`                          |
| `/operator/organizations/$tenantKey`          | `Acme Corp \| Organizations \| Key Value Admin`             |
| `/operator/organizations/$tenantKey/members`  | `Members \| Acme Corp \| Organizations \| Key Value Admin`  |
| `/operator/organizations/$tenantKey/billing`  | `Billing \| Acme Corp \| Organizations \| Key Value Admin`  |
| `/operator/organizations/$tenantKey/activity` | `Activity \| Acme Corp \| Organizations \| Key Value Admin` |
| `/operator/subscriptions`                     | `Subscriptions \| Key Value Admin`                          |
| `/operator/subscriptions/$id`                 | `sub_xxx \| Subscriptions \| Key Value Admin`               |
| `/operator/plans`                             | `Plans \| Billing \| Key Value Admin`                       |
| `/operator/plans/$planCode`                   | `Pro Monthly \| Plans \| Billing \| Key Value Admin`        |
| `/operator/system/health`                     | `Health \| System \| Key Value Admin`                       |
| `/operator/system/jobs`                       | `Jobs \| System \| Key Value Admin`                         |
| `/operator/audit-log`                         | `Audit Log \| Key Value Admin`                              |
...
The SaaS Platform UI provides full internationalization support using **Lingui** for multi-language admin interfaces, enabling platform operators worldwide to use the admin panel in their preferred language.
...
- Operator routes (`/operator/*`) are protected by a single `beforeLoad` guard on the `_operator.tsx` layout route. The guard checks `PLATFORM_OPERATOR` authority from the decoded JWT in the `operator-session` Zustand store.
...
- IP whitelist can be applied at the API Gateway level for `/api/v1/iam/operator/*` and `/api/v1/billing/operator/*` endpoints — this provides network-level isolation without requiring a separate deployment.
...
The operator surface is **not a separate application**. It is a protected route group (`/operator/*`) embedded in the same SPA as the tenant-facing UI. Both surfaces share one build, one deployment, and one API Gateway connection.
...
- **Session stores** — two separate Zustand stores: `session` (tenant user) and `operator-session` (platform operator + impersonation state). Neither store is accessible to the other surface's components.
- **API Gateway** — `/api/v1/iam/operator/*` and `/api/v1/billing/operator/*` endpoints require `PLATFORM_OPERATOR` authority server-side regardless of what the UI sends.
...
│
├── _operator.tsx                       # Pathless layout: operator AppShell + PLATFORM_OPERATOR guard
├── _operator.index.tsx                 # /operator → metrics dashboard
├── _operator.users.index.tsx           # /operator/users
├── _operator.users.$userId.tsx         # /operator/users/:userId
├── _operator.organizations.index.tsx   # /operator/organizations
├── _operator.organizations.$tenantKey.tsx
├── _operator.subscriptions.index.tsx   # /operator/subscriptions
├── _operator.subscriptions.$id.tsx
├── _operator.plans.index.tsx           # /operator/plans
├── _operator.plans.$planCode.tsx
├── _operator.audit-log.tsx             # /operator/audit-log
└── _operator.system.tsx                # /operator/system
```
...
The `beforeLoad` guard on `_operator.tsx` covers every route under `/operator/*` — individual operator pages do not repeat the authority check.
...
├── operator-shell/      # Operator AppShell: operator nav, impersonation banner, Cmd+K palette
├── tenant-dashboard/    # Tenant-facing dashboard (if applicable)
├── operator-dashboard/  # Operator metrics cards, charts, date range selector
...
- The `session` entity holds the current tenant user's JWT state. The `operator-session` entity holds the platform operator's JWT state and impersonation state. These are two separate Zustand stores — neither is accessible from the other surface's components.
...
- The `PLATFORM_OPERATOR` authority check is performed once in `_operator.tsx`'s `beforeLoad`. This single guard covers every route under `/operator/*`. Individual operator pages do not repeat the check.
- Non-authenticated users hitting any protected route are redirected to `/login`. Authenticated users without `PLATFORM_OPERATOR` authority hitting `/operator/*` are redirected to `/` (tenant dashboard).
...
| `_operator.tsx` layout contains `beforeLoad` guard | File content check for `beforeLoad` and `PLATFORM_OPERATOR` |
| Operator pages live under `_operator` prefix       | Filename prefix check for all `/operator/*` routes          |
