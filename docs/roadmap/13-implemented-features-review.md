# Implemented Features Review — June 2026

> Comprehensive snapshot of everything built and shipped across the platform as of June 2026.
> Sources: All service READMEs, source code exploration, and prior roadmap review documents.
> Supersedes document 09 (v0.2 review) and 06 (May review) as the authoritative current state.
...
**Tech stack:** Java 25 / Spring Boot 4.1 · MyBatis 3.x · PostgreSQL 17 · Liquibase · RabbitMQ · JJWT 0.13 RS256 · ShedLock 7.x · Spring WebSocket/STOMP · MinIO S3 · Thymeleaf · Micrometer + Prometheus
...
Plans are defined in YAML (`application-{env}.yml`) and synchronized with Stripe at startup by `BillingSeedRunner`. No REST API for plan mutations — all catalog changes go through configuration and deployment.
...
| Model            | Stripe line item       | `priceMinor` meaning              |
| ---------------- | ---------------------- | --------------------------------- |
| `FLAT` (default) | `quantity = 1` always  | Total price per billing period    |
...
| `POST /subscriptions/{tenantKey}/checkout`                | `TENANT_OWNER`    | Create Stripe Checkout Session                       |
...
| `POST /settings/{tenantKey}/portal`                               | `TENANT_OWNER`   | Create Stripe Customer Portal session |
...
`POST /webhooks/stripe` — public; verifies `Stripe-Signature`; idempotent via `webhook_log` table.

Handles: `customer.subscription.created/updated/deleted`, `invoice.payment_succeeded`, `invoice.payment_failed`.

Events also handled: `invoice.created`, `invoice.finalized`, `invoice.updated`.

#### Event-Driven Integrations

- Consumes `tenant.created/provisioned` → creates Stripe customer, stores `external_customer_id`
...
- Publishes `subscription.created`, `subscription.cancelled`, `invoice.paid/created/finalized/updated`, `payment.failed`, `refund.created`
...
- **`foundation-audit-model`**: `AuditEvent`, `AuditActor`, `ActivityAction`, `ActivitySeverity` enums
- **`foundation-audit-spi`**: `AuditContextHolder`, `AuditContextInterceptor`, `AuditProvider` interface for extensible backends
...
`/sign-in` · `/signup` · `/forgot-password` · `/reset-password` · `/verify-email` · `/invite/:token` · `/create-organization` · `/` (dashboard) · `/team` · `/billing` · `/account/settings/general` · `/account/settings/organization` · `/account/settings/security` · `/account/settings/notifications` · `/unauthorized` · `/404` · `/500`
...
VitePress-based documentation site with user guides, platform overview, and quick start instructions. CI pipeline: `VerifyCode` + `PublishArtifacts` only (no deployment pipeline).
...
- `PlanResolver` (non-reactive, RestTemplate-based) for plan feature lookups
