...
- Replace the current opaque `featureSet` JSON string on plans with a typed, validated
  `PlanFeatures` struct bound directly from YAML.
...
| Module                       | Role                                                                                                          |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `foundation-billing-service` | Source of truth — typed `PlanFeatures`, `PlanFeatureRegistry`, internal plans endpoint, entitlements endpoint |
...
HTTP Request ──► Gateway JwtContextPropagationFilter
                  ...
                  RequiresPlanFeatureFilter
                         │  planCatalogCache.forPlan(planCode).has("priority_support")
                  ...
                  Downstream Service
                    ...
                    planCatalogCache.forPlan(planCode).maxUsers()
                    vs current DB count → 402 if exceeded
```

### 3. `PlanFeatures` — typed struct (`foundation-billing-service`)
...
public class PlanFeatureRegistry {
  ...
  public PlanFeatures forPlan(final String planCode) {
    if (planCode == null) return PlanFeatures.NONE;
...
`PlanFeatures` and `PlanCatalogEntry` are local records in each consumer service —
...
public class RequiresPlanFeatureFilterFactory
  ...
  public GatewayFilter apply(final Config config) {
    return (exchange, chain) -> {
      ...
      if (!planCatalogCache.forPlan(planCode).has(config.getFeature())) {
...
```java
final PlanFeatures features = planCatalogCache.forPlan(
...
- No per-tenant feature overrides — if needed later, add an override table in
  billing-service and merge it in `PlanFeatureRegistry.forPlan()`. The internal
...
- No `foundation-billing-spi` shared library — `PlanCatalogCache` and the local
  `PlanFeatures` record are small, per-service copies. Extract to a shared lib only
